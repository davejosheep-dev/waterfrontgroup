...
export function PublicTableBookingFlow() {
  ...
  async function chooseTime(time: string) {
    ...
    try {
      const response = await fetch("/api/v1/public/venues/waterfront-seafood-cocktails/holds", { method: "POST", headers: { "Content-Type": "application/json", "Idempotency-Key": crypto.randomUUID() }, body: JSON.stringify({ date, time, partySize }) });
      const payload = await response.json();
