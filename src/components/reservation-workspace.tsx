  ...
  return <div className="min-h-screen bg-background text-foreground">
      ...
      <div className="flex h-full flex-col">
        ...
        <div className="border-t border-border px-3 pb-3 pt-2">
          ...
          <button aria-label="Open profile" title={sidebarCollapsed ? "Profile & security" : undefined} onClick={() => openScreen("profile")} className={cx("flex min-h-11 w-full items-center gap-3 rounded-md border border-transparent p-2 text-left transition hover:bg-secondary", sidebarCollapsed && "lg:justify-center")}>
            ...
            <div className={cx("min-w-0 flex-1", sidebarCollapsed && "lg:sr-only")}><div className="truncate text-xs font-semibold text-foreground">{accessContext.fullName}</div><div className="truncate text-[11px] text-muted-foreground">{roleDetails[accessContext.role].label} Â· {accessContext.conceptName}</div></div><MoreHorizontal size={16} className={cx("text-muted-foreground", sidebarCollapsed && "lg:hidden")} />
          </button>
    ...
    <div className={cx("transition-[padding] duration-200", sidebarCollapsed ? "lg:pl-[72px]" : "lg:pl-[248px]")}>
      <header className="sticky top-0 z-30 flex h-[60px] items-center gap-3 border-b border-border bg-background/95 px-4 backdrop-blur md:px-6">
        ...
        <button aria-label="Open global search" onClick={() => setCommandOpen(true)} className="mx-auto flex h-10 min-w-10 max-w-lg flex-1 items-center gap-2 rounded-md border border-border bg-card px-3 text-left text-sm text-muted-foreground hover:bg-secondary"><Search size={16} /><span className="hidden flex-1 sm:inline">Search guests, reservations, and settings</span><kbd className="hidden rounded border border-border bg-secondary px-1.5 py-0.5 text-[10px] font-medium md:inline">âŒ˜K</kbd></button>
        <div className="flex shrink-0 items-center gap-1.5">
          <span className="mr-1 hidden rounded-md border border-border bg-card px-2 py-1 text-[10px] font-medium text-muted-foreground xl:inline">{accessContext.isDemo ? "Demo data" : "Secure"} Â· {currentVenue?.timezone}</span>
...
const tableKindConfig: Record<FloorTableKind, { seats: number; width: number; height: number; description: string }> = {
  T1: { seats: 2, width: 6.5, height: 9, description: "70 Ã— 80 Â· tulip stand" },
  T2: { seats: 4, width: 8, height: 13, description: "80 Ã— 120 Â· normal stand" },
  T3: { seats: 4, width: 7.5, height: 10, description: "80 Ã— 80 Â· tulip stand" },
};
...
function FloorPlanScreen({ reservations, onSelect, notify, canConfigure, canOperate, accessContext }: { reservations: Reservation[]; onSelect: (r: Reservation) => void; notify: (message: string) => void; canConfigure: boolean; canOperate: boolean; accessContext: AccessContext }) {
  ...
  function rotateSelection() {
    ...
    notify(`${ids.length > 1 ? "Selected tables" : selectedTable?.label} rotated 45Â°.`);
  }
    ...
    <div className="grid min-h-[720px] overflow-hidden rounded-lg border border-[#dbe0db] bg-card shadow-soft lg:grid-cols-[300px_1fr]">
      <aside className="border-b border-[var(--border)] bg-[#fbfaf6] lg:border-b-0 lg:border-r">
        {isEditing && <div className="flex h-full flex-col">
          ...
          <div className="flex-1 overflow-y-auto p-4 scrollbar-thin">
            ...
            {!mergeMode && selectedTable && <div className="mt-3 rounded-lg border border-[var(--border)] bg-card p-3"><div className="flex items-center justify-between"><span className="text-[10px] font-bold text-[var(--secondary-foreground)]">{selectedTable.label} selected</span><span className="text-[9px] text-[var(--muted-foreground)]">{selectedTable.rotation}Â°</span></div><div className="mt-3 grid grid-cols-2 gap-2"><button onClick={rotateSelection} className="rounded-lg border border-[var(--input)] px-2 py-2 text-[9px] font-bold uppercase tracking-wide text-[#586763]">Rotate 45Â°</button><button onClick={removeSelectedTable} className="rounded-lg border border-red-100 px-2 py-2 text-[9px] font-bold uppercase tracking-wide text-red-600">Remove</button></div></div>}
            ...
            <div className="space-y-2">{combinations.length ? combinations.map((combination) => <div key={combination.id} className="rounded-lg border border-[var(--border)] bg-card p-3"><div className="flex items-start justify-between gap-2"><div><div className="text-[10px] font-bold text-[var(--secondary-foreground)]">{combination.name}</div><div className="mt-1 text-[9px] text-[var(--muted-foreground)]">{combination.capacity} seats Â· moves together</div></div><button aria-label={`Separate ${combination.name}`} onClick={() => removeCombination(combination)} className="rounded-lg p-1.5 text-[#a06a21] hover:bg-orange-50"><X size={13} /></button></div></div>) : <div className="rounded-lg bg-[var(--secondary)] p-3 text-[10px] text-[var(--muted-foreground)]">No merged table combinations.</div>}</div>
          </div>
        ...
        <div className={cx("border-b border-[var(--border)] p-4", isEditing && "hidden")}>
          <div className="flex items-center justify-between"><div><h2 className="font-display text-xl text-[#243c39]">Reservation queue</h2><p className="mt-1 text-[10px] uppercase tracking-[.08em] text-[var(--muted-foreground)]">{queue.length} bookings Â· 32 covers</p></div><button className="rounded-lg border border-[var(--input)] bg-card p-2 text-[#64716e]"><Filter size={15} /></button></div>
          <div className="mt-4 flex h-10 items-center gap-2 rounded-lg border border-[var(--input)] bg-card px-3"><Search size={15} className="text-[var(--muted-foreground)]" /><input aria-label="Search reservation queue" value={query} onChange={(e) => setQuery(e.target.value)} placeholder="Guest, table or timeâ€¦" className="w-full border-0 bg-transparent text-xs outline-none" /></div>
        ...
        <div className={cx("max-h-[535px] overflow-y-auto p-2 scrollbar-thin", isEditing && "hidden")}>
          ...
          {queue.map((r) => <button key={r.id} onClick={() => onSelect(r)} className="group mb-1 grid w-full grid-cols-[52px_1fr_34px] items-center gap-2 rounded-lg border border-transparent px-2 py-3 text-left transition hover:border-[#e4d4bc] hover:bg-[#fff8ed]">
            ...
            <div className="min-w-0"><div className="truncate text-xs font-bold text-[#334642]">{r.guestName}</div><div className="mt-1 flex items-center gap-1.5 text-[10px] text-[var(--muted-foreground)]"><Users size={11} /><span>{r.guestCount}</span><span>Â·</span><span>{r.table ?? "Unassigned"}</span></div></div>
      ...
      <section className="relative flex min-w-0 flex-col bg-[var(--foreground)]">
        ...
        <div ref={canvasRef} onPointerMove={handleCanvasPointerMove} onPointerUp={handleCanvasPointerUp} onPointerCancel={handleCanvasPointerUp} className={cx("relative min-h-[620px] flex-1 overflow-hidden grid-fade", isEditing && "touch-none cursor-crosshair")}>
          ...
          <div className="absolute bottom-[4%] left-[7%] h-[10%] w-[31%] rounded-t-[28px] border border-white/10 bg-[#1c2529] text-center text-[9px] font-bold uppercase tracking-[.15em] text-white/35"><span className="relative top-4">Cocktail bar Â· 8 stools</span></div>
          ...
          <button className="absolute bottom-[3%] right-[7%] rounded-lg border border-[#8ed7c5]/30 bg-[var(--primary)]/15 px-3 py-2 text-[9px] font-bold uppercase tracking-[.12em] text-[#9edbcf]">VIP Room â†’</button>
        ...
        {isEditing ? <div className="flex flex-col gap-3 border-t border-white/10 bg-[#1d272b] px-4 py-3 text-white sm:flex-row sm:items-center sm:justify-between"><div><div className="text-xs font-bold">{mergeMode ? `${selectedIds.length} tables selected for merging` : selectedIds.length > 1 ? `${selectedIds.length} combined tables selected` : selectedTable ? `${selectedTable.label} Â· drag to reposition` : "Select a table"}</div><div className="mt-1 text-[9px] uppercase tracking-[.1em] text-white/40">{layoutTables.length} physical tables Â· {combinations.length} combinations Â· 60 guest area limit</div></div><div className="flex gap-2">{mergeMode && <button disabled={selectedIds.length < 2} onClick={createCombination} className="rounded-lg border border-white/15 px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-white/75 disabled:opacity-40">Merge selected</button>}<button onClick={saveLayout} className="rounded-lg bg-[var(--primary)] px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-white">Save setup</button></div></div> : selectedTable && <div className="flex flex-col gap-3 border-t border-white/10 bg-[#1d272b] px-4 py-3 text-white sm:flex-row sm:items-center sm:justify-between"><div className="flex items-center gap-3"><div className={cx("flex h-10 w-10 items-center justify-center rounded-lg border text-xs font-black", floorStateStyle[selectedTable.state])}>{selectedTable.label}</div><div><div className="text-xs font-bold">{selectedTable.guest ?? `${selectedTable.seats}-seat table available`}</div><div className="mt-0.5 text-[10px] text-white/45">{selectedTable.time ? `${selectedTable.time} Â· ` : ""}{selectedTable.seats} seats Â· <span className="capitalize">{selectedTable.state}</span></div></div></div><div className="flex gap-2">{selectedReservation && <button onClick={() => onSelect(selectedReservation)} className="rounded-lg border border-white/15 px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-white/75">View booking</button>}<button onClick={() => notify(selectedTable.state === "available" ? `${selectedTable.label} is ready for assignment.` : `${selectedTable.label} service actions opened.`)} className="rounded-lg bg-[var(--primary)] px-3 py-2 text-[10px] font-bold uppercase tracking-wide text-white">{selectedTable.state === "available" ? "Assign table" : "Service actions"}</button></div></div>}
      </section>
    </div>
  </div>;
}
...
function TodayScreen({ reservations, onSelect, onNew, onAvailability }: { reservations: Reservation[]; onSelect: (r: Reservation) => void; onNew: () => void; onAvailability: () => void }) {
  ...
  return <div className="animate-rise">
    <PageHeader eyebrow="Friday Â· 07 August 2026" title="Todayâ€™s reservations" description="A clear view of service, confirmations, and deposits for Waterfront Seafood & Cocktails.">
    ...
    <div className="grid gap-4 px-5 py-6 md:grid-cols-2 md:px-8 xl:grid-cols-4">
      ...
      <Metric label="Needs attention" value="03" note="2 holds Â· 1 deposit" icon={<Bell size={18} />} accent />
    </div>
      ...
      <div className="overflow-hidden rounded-lg border border-[var(--border)] bg-[var(--card)] shadow-soft">
        ...
        <div className="divide-y divide-[var(--border)]">
          {reservations.map((r) => <button key={r.id} onClick={() => onSelect(r)} className="grid w-full gap-3 px-5 py-4 text-left transition hover:bg-[#fffaf2] lg:grid-cols-[90px_1.5fr_100px_1.15fr_130px_140px_38px] lg:items-center">
            ...
            <div className="min-w-0"><div className="truncate text-sm font-bold text-[#263b38]">{r.guestName}</div><div className="mt-0.5 flex items-center gap-1.5 truncate text-[11px] text-[var(--muted-foreground)]"><span>{r.code}</span>{r.occasion && <><span>Â·</span><span>{r.occasion}</span></>}</div></div>
